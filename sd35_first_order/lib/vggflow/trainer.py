...
class VGGFlowTrainer:
    ...
    def _train_single_timestep(
        ...
        velocity_target, loss_components = self.algorithm.compute_velocity_target(
            ...
            saved_rgrad_x1=batch.get("rgrad_x1"),
        ...
        transition_drift = self.algorithm.compute_transition_drift(
            sigma=sigma,
            step_index=batch["step_index"][:, step_idx],
            velocity_current=velocity_current,
            velocity_reference=loss_components["velocity_target_raw"],
        )
        self.info["transition_drift"].append(transition_drift.detach())
