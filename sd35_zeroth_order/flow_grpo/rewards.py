...
def aesthetic_score():
    ...
    def _fn(images, prompts, metadata):
        ...
        if isinstance(images, torch.Tensor):
            ...
            if images.ndim != 4:
                raise ValueError(
                    f"aesthetic reward expects rank-4 image batches, got shape {images.shape}."
                )
            if images.shape[-1] in (1, 3, 4):
                images = torch.tensor(images, dtype=torch.uint8).permute(0, 3, 1, 2)
            elif images.shape[1] in (1, 3, 4):
                images = torch.tensor(images, dtype=torch.uint8)
            else:
                raise ValueError(
                    "aesthetic reward expects NHWC or NCHW image arrays, got "
                    f"shape {images.shape}."
                )
        else:
            if not isinstance(images, (list, tuple)):
                raise TypeError(
                    "aesthetic reward expects images as torch.Tensor, np.ndarray, or list/tuple, got "
            ...
            for image in images:
                ...
                if image.ndim != 3:
                    raise ValueError(
                        "aesthetic reward expects each image to be rank-3 (HWC or CHW), got "
                        f"shape {image.shape}."
                    )
                if image.shape[-1] in (1, 3, 4):
                    ...
                    raise ValueError(
                        "aesthetic reward expects HWC or CHW images with 1/3/4 channels, got "
                        f"shape {image.shape}."
                    )
                converted.append(chw.astype(np.uint8, copy=False))
            images = torch.tensor(np.stack(converted, axis=0), dtype=torch.uint8)
...
def hpsv3_score_remote(device):
    ...
    batch_size = 64
    url = os.getenv("TEMPFLOW_HPSV3_REWARD_URL", "http://127.0.0.1:18086")
...
def deqa_score_remote(device):
    ...
    batch_size = 64
    url = os.getenv("TEMPFLOW_DEQA_REWARD_URL", "http://127.0.0.1:18086")
    sess = requests.Session()
    retries = Retry(
        total=1000, backoff_factor=1, status_forcelist=[500], allowed_methods=False
    )
...
def geneval_score(device):
    ...
    batch_size = 64
    url = os.getenv("TEMPFLOW_GENEVAL_REWARD_URL", "http://127.0.0.1:18085")
    sess = requests.Session()
    retries = Retry(
        total=1000, backoff_factor=1, status_forcelist=[500], allowed_methods=False
    )
...
def unifiedreward_score_remote(device):
    ...
    batch_size = 64
    url = os.getenv("TEMPFLOW_UNIFIEDREMOTE_REWARD_URL", "http://10.82.120.15:18085")
    sess = requests.Session()
    retries = Retry(
        total=1000, backoff_factor=1, status_forcelist=[500], allowed_methods=False
    )
...
def multi_score(device, score_dict):
    ...
    for score_name, weight in score_dict.items():
        if score_name not in score_functions:
            raise ValueError(
                f"Unsupported reward key `{score_name}`. "
                f"Supported rewards: {', '.join(sorted(score_functions.keys()))}"
            )
