...
def hps_v2():
    ...
    def _fn(images, prompts, metadata):
        ...
        with torch.amp.autocast('cuda', enabled=(device.type == 'cuda')):
            ...
            scores = torch.diagonal(logits_per_image).contiguous() # [B]
...
def hpsv2_score_diff(
    ...
    """
    Differentiable HPSv2 reward.
    Returns a torch tensor so gradients can flow to the input images.
    """
    import warnings
    ...
    def _fn(images, prompts, metadata):
        ...
        if isinstance(images, torch.Tensor):
            ...
            imgs = torch.as_tensor(images)
            if imgs.ndim != 4:
                raise ValueError(f"Expected a 4D image batch, got shape={tuple(imgs.shape)}")
            # NHWC -> NCHW for numpy input.
            if imgs.shape[-1] in (1, 3, 4):
                imgs = imgs.permute(0, 3, 1, 2)

        if imgs.ndim != 4:
            raise ValueError(f"Expected a 4D image batch, got shape={tuple(imgs.shape)}")
        if len(prompts) != imgs.shape[0]:
            raise ValueError(
                f"Expected {imgs.shape[0]} prompts, but got {len(prompts)}"
            )
        ...
        image_batch = torch.stack([hps_preprocess(img) for img in imgs], dim=0)
        text_batch = tokenizer(prompts).to(device=device, non_blocking=True)
        ...
        with amp_ctx:
            ...
            logits_per_image = image_features @ text_features.T
            scores = torch.diagonal(logits_per_image).contiguous()
...
def aesthetic_score_diff(dtype=torch.float32, device="cuda", distributed=True):
    from utils.aesthetic_scorer import AestheticScorerDiff

    scorer = AestheticScorerDiff(dtype=dtype, distributed=distributed).to(device)
